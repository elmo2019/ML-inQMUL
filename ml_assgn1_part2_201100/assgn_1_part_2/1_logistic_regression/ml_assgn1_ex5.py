...
colums_triplex1 = np.array([],dtype=np.float32)
colums_triplex2 = np.array([],dtype=np.float32)
for i in range(X.shape[0]):
    ...
    colums_triplex1 = np.append(colums_triplex1,curren_Value)
    curren_Value = X[i, 1] * X[i, 1] * X[i, 1]
    colums_triplex2 = np.append(colums_triplex2, curren_Value)
X=np.c_[X,columns_x1x2]
X=np.c_[X,columns_doublex1]
X=np.c_[X,columns_doublex2]
X=np.c_[X,colums_triplex1]
X=np.c_[X,colums_triplex2]
########################################/
